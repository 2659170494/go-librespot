...
echo "如果你需要在termux上运行，请编译和你手机一模一样的架构！！！"
echo "关于pulseaudio，你可以下载Simple Protocol Player然后设置ip和端口并打开网络连接进行对接，"
echo "或者下载Termux和Termux:API，安装完成之后在Termux执行pkg install sox termux-x11 termux-api pulseaudio play-audio"
...
# echo "之后执行：pulseaudio -k && pulseaudio --start --exit-idle-time=-1 && pacmd load-module module-sles-sink && pacmd load-module module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1 "
# echo "然后重启termux，再执行一遍上面的命令。回到shell里执行后即可食用：export PULSE_SERVER=tcp:127.0.0.1:4713"
echo "之后执行这条命令：export PRRFIX=/data/data/com.termux/files/usr && vi \$PREFIX/etc/pulse/default.pa "
echo "在vi页面中按下键盘的i键，并将光标移动到第一个（即左上方）"
echo "之后输入以下内容："
echo "load-module module-sles-sink"
echo "load-module module-native-protocol-tcp auth-ip-acl=127.0.0.1[;本地局域网ip,如需外部访问,可选]/[范围,可选] auth-anonymous=1"
echo "（例如：load-module module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1）"
echo "完成之后按下esc，之后输入:wq回车退出并保存"
echo "之后再执行pulseaudio -k;pulseaudio -D"
echo "最后将termux小窗但不要放后台或杀后台（放前台也可以），"
echo "回到shell里执行后即可食用：export PULSE_SERVER=tcp:&lt;Pulseaudio服务器地址&gt;[:端口,默认4713]"
echo "（例如：export PULSE_SERVER=tcp:127.0.0.1:4713）"
